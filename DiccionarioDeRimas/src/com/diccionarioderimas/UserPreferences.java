...
public class UserPreferences {
	...
	public UserPreferences(String basePath) {
		...
		try {
			...
			if (lastUp != null)
				lastUpdated = new Date(Long.parseLong(lastUp));
			if (the != null)
				theme = the;
			if (yEquival != null)
			...
			if (counter != null)
				viewCounter = Boolean.parseBoolean(counter);
			if (top != null)
				stayOnTop = Boolean.parseBoolean(top);
			if (proxy != null)
	...
	public void save() {
		...
		try {
			...
			prop.store(
					new FileOutputStream(basePath + "preferences.properties"),
					null);
		} catch (Exception e) {
			e.printStackTrace();
		}
