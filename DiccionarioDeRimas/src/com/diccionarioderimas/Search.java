...
public class Search {
	...
	public Search(String word, int rhymeType, int silables, int beggining,
		...
		UserPreferences preferences = MainWindow.getUserPreferences();
		...
		for (int i = 0; i < 3; i++) {
			...
			Vector<FileToSearch> files = getFiles(i, finder.getAsonance(), basePath);
			for (Iterator iterator = files.iterator(); iterator.hasNext();) {
				FileToSearch fs = (FileToSearch) iterator.next();
				boolean hasRhyme=(rhymeType==ASONANCE || fs.fits(finder.getRhyme()) );
				if(rhymeType!=ASONANCE){
					if (!hasRhyme && preferences.isCeEquivalence()){
						hasRhyme= fs.fits(finder.getRhyme().replace("ce", "se").replace("ci", "si"));
						if(!hasRhyme)
							hasRhyme= fs.fits(finder.getRhyme().replace("se", "ce").replace("si", "ci"));
					}
					if (!hasRhyme && preferences.isLiEquivalence()){
						hasRhyme=fs.fits( finder.getRhyme().replace("ll", "li"));
						if(!hasRhyme)
							hasRhyme= fs.fits(finder.getRhyme().replace("li", "ll"));
					}
					if (!hasRhyme && preferences.isNiEquivalence()){
						hasRhyme=fs.fits(finder.getRhyme().replace("ñi", "ni"));
						if(!hasRhyme)
							hasRhyme= fs.fits(finder.getRhyme().replace("ni", "ñi"));
					}
					if (!hasRhyme && preferences.isYEquivalence()){
						hasRhyme= fs.fits(finder.getRhyme().replace("y", "ll"));
						if(!hasRhyme)
							hasRhyme= fs.fits(finder.getRhyme().replace("ll", "y"));
					}
					if (!hasRhyme && preferences.isZEquivalence()){
						hasRhyme= fs.fits(finder.getRhyme().replace("z", "s"));
						if(!hasRhyme)
							hasRhyme= fs.fits(finder.getRhyme().replace("s", "z"));
					}
				}
				if (fs.getFile().exists() && hasRhyme) {
					
					try {
						BufferedReader in = new BufferedReader(new FileReader(fs.getFile()));
						String line = null;
						while ((line = in.readLine()) != null) {
							...
							if (filter) {
								if (silables != INDIFERENT && silables != silCount)
									filter = false;
								else if (beggining != INDIFERENT
										&& beggining != Integer.parseInt(parts[5]))
									filter = false;
							}
