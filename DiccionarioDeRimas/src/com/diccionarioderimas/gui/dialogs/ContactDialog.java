...
public class ContactDialog extends JDialog implements ActionListener {
	...
	public ContactDialog(JFrame parent) {
		...
		name = new JTextField();
		category = new JComboBox(new String[] { "Comentario", "Sugerencia",
				"Problema" });
		...
		JLabel categoryL = new JLabel("Categoría:");
		JLabel subjectL = new JLabel("Título:");
		...
		categoryL.setBounds(100 - categoryL.getPreferredSize().width, 75, 100,
				30);
		subjectL.setBounds(100 - subjectL.getPreferredSize().width, 105, 100,
				30);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == cancel) {
			...
			if (subject.getText().trim().equals(""))
				errors.append("El campo 'Título' es obligatorio<br/>");
			if (message.getText().trim().equals(""))
				errors.append("No hay conrtenido en el mensaje<br/>");
			if (errors.length() > 0) {
				JOptionPane.showMessageDialog(this, "<html><b>Error</b><br/>"
						+ errors.toString() + "</html>", "Error",
				...
				try {
					...
					params
							.append("name=")
							.append(URLEncoder.encode(name.getText(), "UTF-8"))
							.append("&category=")
							.append(
									URLEncoder.encode((String) category
											.getSelectedItem(), "UTF-8"))
							.append("&title=")
							.append(
									URLEncoder.encode(subject.getText(),
											"UTF-8"))
							.append("&email=")
							.append(URLEncoder.encode(email.getText(), "UTF-8"))
							.append("&id=").append(
									URLEncoder.encode(MainWindow
											.getUserPreferences().getUniqueId()
											+ "", "UTF-8")).append("&body=")
							.append(
									URLEncoder.encode(message.getText(),
											"UTF-8"));

					URL url = new URL(
							"http://www.cronopista.com/diccionario2/sendMessage.php");
					...
					JOptionPane
							.showMessageDialog(
									...
									"<html><b>Error</b><br/>Ha ocurrido un error enviando tu mensaje.<br/>"
											+ "Por favor, inténtalo más tarde o ponte en contacto conmigo a través de www.cronopista.com</html>",
